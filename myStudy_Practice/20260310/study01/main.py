...
def setData2(data:dict):
    ...
    sql_ins = f'INSERT INTO `db_to_air`.`{table}` select * from `db_air`.{table} {where}'
    if table == '비행':
        sql_ins = f'INSERT INTO `db_to_air`.`{table}` select * from `db_air`.`뷰_비행` {where}'
    sql_selc1 = f'select count(*) as cnt from `db_air`.`{table}` {where}'
    sql_selc2 = f'select  count(*) as cnt from `db_to_air`.`{table}` {where}'
...
@app.post('/selectData')
def selectData(year: int, month:tuple[int,...]=()):
    print(month)
    with mariadb.connect(**conn_params) as conn:
        select_sql = f'UPDATE `db_to_air`.`jobs` SET `useYn` = 1 WHERE year = {year} and month in {month}'
        with conn.cursor() as cursor:
...
@app.post('/data')
def data():
    sql_del =  f'TRUNCATE TABLE `db_to_air`.`비행`'
    with mariadb.connect(**conn_params) as conn:
        with conn.cursor() as cursor:
            cursor.execute(sql_del)
